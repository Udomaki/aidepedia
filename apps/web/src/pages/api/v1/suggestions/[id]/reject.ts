...
import { rejectEditSuggestion } from '@aidepedia/db';
...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    // Reject suggestion
    const suggestion = await rejectEditSuggestion(parseInt(id));
    ...
    console.error('Reject edit suggestion error:', error);
    ...
    return new Response(
      JSON.stringify({ error: error.message || 'Failed to reject edit suggestion' }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      }
